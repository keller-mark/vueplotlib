<template>
    <div>
        <svg 
            :id="this.hierarchyElemID" 
            class="vdp-plot" 
            :style="{
                'height': (this.pMarginTop) + 'px', 
...
export default {
    ...
    computed: {
        hierarchyElemID: function() {
            return 'hierarchy_' + this.uuid;
        },
        hierarchySelector: function() {
            return "#" + this.hierarchyElemID;
        }
    },
    data() {
        return {
    ...
    created() {
        ...
        this._hierarchyDataContainer = this.getData(this.h);
        console.assert(this._hierarchyDataContainer instanceof DataContainer);
        // Set scale variables
        ...
        this._hierarchyDataContainer.onUpdate(this.uuid, this.drawPlot);
    ...
    methods: {
        ...
        drawPlot() {
            ...
            let hierarchyData = this._hierarchyDataContainer.dataCopy;
            ...
            const filterNode = (node) => {
                if(node.children.length === 0) {
                    if(xScale.domainFiltered.includes(node.name)) {
                        return node;
                    } else {
                        return undefined;
                    }
            ...
            const hierarchyDataFiltered = filterNode(hierarchyData);

            /*
                Prepare the hierarchy
            */
            const size = [vm.pWidth, vm.pMarginTop]; 
            // TODO: allow for dendrogram on side, bottom ,etc...

            const tree = d3_cluster()
                .size(size)
                .separation(() => 1);
            const root = d3_hierarchy(hierarchyDataFiltered);
            tree(root);

            const leaves = root.leaves();
            // Use leaves to sort x-axis by hierarchy since any other ordering does not make sense here
            const xDomainSortedByLeaf = leaves.map((el) => el.data.name);

            /*
                Draw the dendrogram
            */
           d3_select(this.hierarchySelector).select("g").remove();
           const hierarchyContainer = d3_select(vm.hierarchySelector);
            
            const gTree = hierarchyContainer.append("g")
